...
namespace wasm {
   ...
   public:
      virtual std::shared_ptr<wasm_instantiated_module_interface> instantiate_module(const char* code_bytes, size_t code_size) = 0;
    ...
    public:
        wasm_vm_runtime();
        std::shared_ptr <wasm_instantiated_module_interface> instantiate_module(const char *code_bytes, size_t code_size) override;
    ...
    //和上边的区别是 get_instantiated_module 的参数是 指针+长度,并且module初次解析后会被缓存下来
    template<typename Backend>
    class wasm_vm_runtime2 : public wasm_runtime_interface{
    public:
        wasm_vm_runtime2();
        std::shared_ptr <wasm_instantiated_module_interface> instantiate_module(const char *code_bytes, size_t code_size, const uint256 &hash);
        void immediately_exit_currently_running_module() override;
        void validate(const vector <uint8_t> &code) override;
