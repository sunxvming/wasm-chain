        ...
        int64_t apply(wasm::wasm_context_interface *pContext) override {
            ...
            auto fn = [&]() {
                const auto &res = _runtime->_bkend->call_with_return(
                        pContext, "env", "apply", i,i, wasm::name(pContext->action()).value);
            ...
            // std::cout << "contract: " << pContext->contract() << " action:" << pContext->action() << " return value:" << apply_return << std::endl;
