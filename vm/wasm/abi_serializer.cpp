...
#include "utils/json/json_spirit_writer.h"
...
namespace wasm {
    ...
    auto pack_unpack() {
        return std::make_pair<abi_serializer::unpack_function, abi_serializer::pack_function>(
                []( wasm::datastream<const char *> &ds, bool is_array, bool is_optional ) -> json_spirit::Value {   // datastream --> variant
                    if (is_array){
                ...
                []( const json_spirit::Value &var, wasm::datastream<char *> &ds, bool is_array, bool is_optional ) {  //wariant --> datastream
                    if (is_array) {
                        vector <T> ts;
    ...
    bool abi_serializer::_is_type( const type_name &rtype, wasm::abi_traverse_context &ctx ) const {
        ...
        //WASM_TRACE("type:%s fundamental_type:%s", rtype, type)
   ...
   json_spirit::Value abi_serializer::get_field_variant( const type_name &s, const json_spirit::Value &v, field_name field, bool is_optional ) const {
    ...
    void abi_serializer::_variant_to_binary( const type_name &type, const json_spirit::Value &var,
        ...
        try {
            ...
            //WASM_TRACE("type:%s rtype:%s fundamental_type:%s", type, rtype, fundamental_type(rtype))

            if (btype != built_in_types.end()) {  //递归基。是内置类型的话，就找到内置类型的序列化的方法， 
                btype->second.second(var, ds, is_array(rtype), is_optional(rtype));   // json-->c++类型 -->datastream
            ...
            } else if ((s_itr = structs.find(rtype)) != structs.end()) {  //在structs中找action的定义
                const auto &st = s_itr->second;   // action的structs的结构
