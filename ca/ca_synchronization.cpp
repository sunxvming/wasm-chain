   ...
   DEBUGLOG("nodeInfos size() = {}", nodeInfos.size());
	...
	else if((type == kAUTOMUTIC)  && (!ispublicid))
	{
		...
		if(self_id.compare(idsequence[0].id))
		{
			...
			DEBUGLOG("potential g_localnode[0].ip = {} g_localnode[0].height = {}", IpPort::ipsz(g_localnode[0].local_ip), g_localnode[0].chain_height);
		}	
	}
	...
	if (txn == NULL)
	{
		ERRORLOG("(HandleSyncVerifyPledgeNodeReq) TransactionInit failed !");
		return ;
	}
	...
	for(auto const &i :reliables)
	{
		...
		else if ((type == kAUTOMUTIC)  && (!ispublicid))
		{
			...
			{
				ca_console infoColor(kConsoleColor_Red, kConsoleColor_Black, true);
				DEBUGLOG("autoid.ip = {}, autoid.height = {}", IpPort::ipsz(g_localnode[0].local_ip), g_localnode[0].chain_height);
...
void SendSyncGetnodeInfoReq(std::string id)
{
	...
	{
		ERRORLOG("(HandleSyncVerifyPledgeNodeReq) GetBlockTop failed !");
		return;
	}
...
void HandleSyncGetnodeInfoReq( const std::shared_ptr<SyncGetnodeInfoReq>& msg, const MsgData& msgdata )
{
	...
	if( txn == NULL )
	{
		ERRORLOG("HandleSyncGetnodeInfoReq TransactionInit");
		return;
	}
...
void HandleSyncBlockInfoAck( const std::shared_ptr<SyncBlockInfoAck>& msg, const MsgData& msgdata )
{
	...
	DEBUGLOG("HandleSyncBlockInfoAck id = {}. ip = {}, syncBlockInfoReq height ={}", id, IpPort::ipsz(node.local_ip), node.chain_height);
	// 判断版本是否兼容
	if( 0 != Util::IsVersionCompatible( pSyncHeaderMsg->version() ) )
	{
...
int SyncData(std::string &headerstr, bool isSync)
{
	...
	if( txn == NULL )
	{
		ERRORLOG("(SyncData) TransactionInit failed !");
		return -1;
	}
