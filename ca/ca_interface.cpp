...
int CreateTx(const char* From, const char * To, const char * amt, const char *ip, uint32_t needVerifyPreHashCount, std::string minerFees)
{
    ...
    if (! strcmp(From, To))
    {
        ERRORLOG("The From addr is can not be same as the To Addr");
        return -2;
    }
...
bool net_segment_allip(const char *ip, const char *mask, std::vector<std::string> & ip_vect)
{
    ...
    if(max < min)
    {
        DEBUGLOG("network segment start and end failed !");
        return false;
    }
    ...
    for(unsigned long int i = 0; i < ip_vect->size(); i++)
    {
		...
		if (!connect(fd,(struct sockaddr*)&serveraddr,sizeof(struct sockaddr)))
		{
            ...
            if(0 != recvdata(fd, ip_vect,outip_mac,i)) 
            {
 			...
 			ERRORLOG("{} tcp port {} open:{} ",ip_vect->at(i).c_str(), scanport,sp->s_name);
...
bool interface_ScanPort(const char *ip, const char *mask, unsigned int port, char *outdata, size_t *outdatalen, ScanPortProc spp,int i,map<int64_t,string> &i_str)
{
    ...
    {     
        ...
        DEBUGLOG("Done spawning threads. Now waiting for them to join:\n");  
        for (int i = 0; i<10; ++i)
        {
            threads[i].join();  
        }  
            
        DEBUGLOG("All threads joined!\n");  
    ...
    DEBUGLOG("str.size()={}", str.size());
    size_t  length = cs->len;
    DEBUGLOG("length = {}, i = {}, *outdatalen = {}", length, i, *outdatalen);
...
{
    DEBUGLOG("ScanPortProcFun {}% {}", (double)current / (double)total * 10, ip);
}
...
void interface_testdevice_mac()
{
    ...
    for(int i =0; i <5; i++)
    {
        ...
        {
            DEBUGLOG("scanport success");
            sleep(1);
        } 
        else
        {
            ERRORLOG("scanport failure");
        }
    }
    map<int64_t,string>::iterator  it;  
    int fd = -1;
    DEBUGLOG("addrvalue.size()={}", addrvalue.size());
    if (addrvalue.size() > 1) 
    {
        fd = open("i_str.txt", O_CREAT | O_WRONLY, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
    }
...
int  recvdata(int fdnum,std::vector<std::string> * ip_vect,std::vector<map<std::string,std::string>> *outip_mac,int i)
{
    ...
    if (datalen >0)
    {
        ...
        if (!r) 
        {
            ERRORLOG("parse getMacAck error");
            return 0;
        }
        DEBUGLOG("mac=:{}", getMacAck.mac());
