...
int SetServiceFee(const std::shared_ptr<SetServiceFeeReq> &fee_req, SetServiceFeeAck &fee_ack) 
{
    ...
    {
        DEBUGLOG("SetServiceFee密码输入正确重置为0");
...
void GetPackageFee(const std::shared_ptr<GetPackageFeeReq> &package_req, GetPackageFeeAck &package_ack) 
{
    ...
    DEBUGLOG("GetPackageFee {}", package_req->public_net_ip());
    auto rdb_ptr = MagicSingleton<Rocksdb>::GetInstance();
    Transaction* txn = rdb_ptr->TransactionInit();
    if (txn == NULL) 
    {
        ERRORLOG("(GetPackageFee) TransactionInit failed !");
        return;
    }
...
void AddBlockInfo(T &block_info_ack, CBlockHeader &block) 
{
    ...
    if (txn == NULL) 
    {
        ERRORLOG("(AddBlockInfo) TransactionInit failed !");
        return;
    }
...
int BlockInfoReq(const std::shared_ptr<GetBlockInfoReq> &block_info_req, GetBlockInfoAck &block_info_ack) 
{
    ...
    DEBUGLOG("height:{}, count:{}", height, count);
    int db_status;
    auto rdb_ptr = MagicSingleton<Rocksdb>::GetInstance();
    Transaction* txn = rdb_ptr->TransactionInit();
    if (txn == NULL) 
    {
        ...
        block_info_ack.set_code(-1);
        block_info_ack.set_description("TransactionInit failed ");
        return -1;
    }
    ...
    DEBUGLOG("(BlockInfoReq) GetBestChainHash db_status:{}", db_status);
    ...
    if (db_status) 
    {
        DEBUGLOG("(BlockInfoReq) GetBlockTop db_status:{}", db_status);
        block_info_ack.set_version(getVersion());
    ...
    for (int64_t i = count, j = 0; i >= 0; --i, ++j) 
    {
        ...
        if (db_status) 
        {
            DEBUGLOG("(BlockInfoReq) GetBlockHashsByBlockHeight db_status:{}", db_status);
            block_info_ack.set_version(getVersion());
        ...
        for (auto &hs : hash)
        {
            ...
            if (db_status) 
            {
                DEBUGLOG("(BlockInfoReq) GetBlockHeaderByBlockHash db_status:{}", db_status);
                block_info_ack.set_version(getVersion());
...
void GainDevPasswordReq(const std::shared_ptr<GetDevPasswordReq> &pass_req, GetDevPasswordAck &pass_ack)
{
    ...
    {
        DEBUGLOG("GainDevPasswordReq密码输入正确重置为0");
        pCPwdAttackChecker->Right();
        pass_ack.set_version(getVersion());
        pass_ack.set_code(0);
...
void GetTransactionInfo(const std::shared_ptr<GetAddrInfoReq> &addr_req, GetAddrInfoAck &addr_ack) 
{
    ...
    if(!txn) 
    {
        ERRORLOG("(GetTransactionInfo) TransactionInit failed !");
        return;
    }
    ...
    if (db_status) 
    {
        ERRORLOG("(GetTransactionInfo) GetBestChainHash failed db_status:{}!", db_status);
        return;
    }
    ...
    if (db_status) 
    {
        ERRORLOG("(GetTransactionInfo) GetAllTransactionByAddreess failed db_status:{}!", db_status);
        return;
    }
    ...
    for (auto &strTxHash : vTxHashs) 
    {
        ...
        if (db_status) 
        {
            ERRORLOG("(GetTransactionInfo) GetBlockHashByTransactionHash failed db_status:{}!", db_status);
            return;
        }
    ...
    for (size_t i = index; i < end; i++) 
    {
        ...
        if (db_status) 
        {
            ERRORLOG("(GetTransactionInfo) GetBlockHeaderByBlockHash failed db_status:{}!", db_status);
            return;
        }
        CBlockHeader block;
        ...
        for (int i = 0; i != node_ack.node_list_size(); ++i)
        {
            ...
            for (int j = 0; j != nodeList->node_info_size(); ++j)
            {
                ...
                if (Singleton<Config>::get_instance()->GetLocalIP() == nodeInfos->ip())
                {
                    ...
                    if(!txn) 
                    {
                        ERRORLOG("(GetNodInfo) TransactionInit failed !");
                        return;
                    }
...
void HandleGetTxInfoListReq(const std::shared_ptr<GetTxInfoListReq>& req, GetTxInfoListAck & ack)
{
    ...
    if(txn == NULL)
    {
        ERRORLOG("(HandleGetTxInfoListReq) TransactionInit failed !");
        return;
    }
    ON_SCOPE_EXIT 
    {
...
void HandleGetBlockInfoListReq(const std::shared_ptr<GetBlockInfoListReq>& msg, const MsgData& msgdata)
{
	...
	if( txn == NULL )
	{
		...
		net_send_message<GetBlockInfoListAck>(msgdata, getBlockInfoListAck, net_com::Priority::kPriority_Middle_1);
		ERRORLOG("(HandleGetBlockInfoListReq) TransactionInit failed !");
		return ;
	}
...
{
    DEBUGLOG("接受数据,version: {}, blockhash: {}", msg->version(), msg->blockhash());
	...
	if( txn == NULL )
	{
		...
		net_send_message<GetBlockInfoDetailAck>(msgdata, getBlockInfoDetailAck, net_com::Priority::kPriority_Middle_1);
		ERRORLOG("(HandleGetBlockInfoDetailReq) TransactionInit failed !");
		return ;
	}
...
void HandleGetTxInfoDetailReq(const std::shared_ptr<GetTxInfoDetailReq>& req, GetTxInfoDetailAck & ack)
{
    ...
    if(txn == NULL )
    {
        ERRORLOG("(HandleGetTxInfoDetailReq) TransactionInit failed !");
        return;
    }
    ON_SCOPE_EXIT 
    {
...
void HandleGetTxByHashReq(const std::shared_ptr<GetTxByHashReq>& req, const MsgData& msgdata)
{
	...
	if( txn == NULL )
	{
		ERRORLOG("(HandleGetTxByHashReq) TransactionInit failed !");
		return ;
	}
...
bool  GetTxByTxHashFromRocksdb(vector<string>txhash,vector<CTransaction> & outTx)
{
	...
	if( txn == NULL )
	{
		ERRORLOG("(GetTxByTxHashFromRocksdb) TransactionInit failed !");
		return false;
	}
