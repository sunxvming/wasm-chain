...
void printRocksdb(uint64_t start, uint64_t end) {
    int fd = -1;
    if (end - start > 5) {
        fd = open("print_blk.txt", O_CREAT | O_WRONLY, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
    }
    auto pRocksDb = MagicSingleton<Rocksdb>::GetInstance();
	Transaction* txn = pRocksDb->TransactionInit();
	if( txn == NULL )
	{
		ERRORLOG("(printRocksdb) TransactionInit failed !");
		return ;
	}
...
std::string printBlocks(int num, bool pre_hash_flag)
{
    ...
    if (txn == NULL)
    {
        ERRORLOG("printBlocks create txn fail! ");
        return "";
    }
