...
int Rocksdb::setKey(const std::string &key, const std::string &value)
{
    return writedata(key,value);
}


std::map<std::string, std::string> Rocksdb::prefix_key(const std::string& key)
{
    ...
    for (iter->Seek(key); iter->Valid() && iter->key().starts_with(key); iter->Next()) {
        res[iter->key().ToString()] = iter->value().ToString();
    }
    return res;
}
...
int Rocksdb::SetContractByAddress(Transaction* txn, const std::string & addr, const std::string & contract, const std::string abi)
{
    if (txn == NULL || addr.empty() || contract.empty() || abi.empty())
    {
        return ROCKSDB_PARAM_NULL;
    }

    std::string contract_key = this->kAddress2Contract + "_" + addr;
    std::string abi_key = this->kAddress2Abi + "_" + addr;

    writedata(txn, contract_key, contract);
    writedata(txn, abi_key, abi);
  
    return ROCKSDB_SUC;
}


int Rocksdb::GetContractByAddress(Transaction* txn, const std::string & addr, std::string & contract, std::string& abi)
{
    if (txn == NULL || addr.empty() )
    {
        return ROCKSDB_PARAM_NULL;
    ...
    int ret = readdata(txn, contract_key, contract);
    if (ret != ROCKSDB_SUC)
    {
        return ret;
    }

    ret = readdata(txn, abi_key, abi);
    if (ret != ROCKSDB_SUC)
    {
        return ret;
    } 
