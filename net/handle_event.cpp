...
void handleRegisterNodeAck(const std::shared_ptr<RegisterNodeAck> &registerNodeAck, const MsgData &from)
{
	...
	DEBUGLOG("handleRegisterNodeAck self_node_id:", self_node.id);
	...
	for (int i = 0; i < registerNodeAck->nodes_size(); i++)
	{
		...
		DEBUGLOG("handleRegisterNodeAck node.id: {}", node.id);
...
void handleTransMsgReq(const std::shared_ptr<TransMsgReq> &transMsgReq, const MsgData &from)
{
	...
	const std::string & nodeid = nodeinfo->node_id();
	const std::string & public_nodeid = nodeinfo->public_node_id();
...
void handleGetHeightReq(const std::shared_ptr<GetHeightReq> &heightReq, const MsgData &from)
{
	...
	int step = len / MAX_NODE_SIZE ;
	if(0 == step)
		step = 1;
	for (size_t i = 0, index = i * step;  index < len && i < MAX_NODE_SIZE; i++, index = i * step)
	{
		NodeHeight *nodeHeight = heightAck.add_nodes_height();
		nodeHeight->set_id(nodelist[index].id);
		nodeHeight->set_height(nodelist[index].chain_height);
		nodeHeight->set_base58addr(nodelist[index].base58address);
		nodeHeight->set_fee(nodelist[index].fee);
	}
